use crate::{errors::PlasmaError, fixed::I80F48, Downcast, Upcast};
...
impl Amm {
    pub fn simulate_buy_exact_in(&self, quote_in: u64) -> Result<SwapResult, PlasmaError> {
        self.simulate_buy_exact_in_with_slot(self.get_slot(), quote_in)
    }

    pub fn simulate_sell_exact_in(&self, base_in: u64) -> Result<SwapResult, PlasmaError> {
...
impl Amm {
    ...
    pub fn get_quote_in_from_base_out(&self, base_out: u128) -> Result<u128, PlasmaError> {
        let base_reserves = self.base_reserves.upcast();
        let quote_reserves = self.quote_reserves.upcast();
        let k = (base_reserves * quote_reserves).saturating_sub(1);
        if base_out >= base_reserves {
            return Err(PlasmaError::SwapOutputGreaterThanOrEqualToReserves(
                base_out,
                base_reserves,
            ));
        }
    ...
    pub fn get_base_in_from_quote_out(&self, quote_out: u128) -> Result<u128, PlasmaError> {
        let base_reserves = self.base_reserves.upcast();
        let quote_reserves = self.quote_reserves.upcast();
        let k = (base_reserves * quote_reserves).saturating_sub(1);
...
impl Amm {
    pub fn mint(
    ...
    ) -> Result<(u64, u64, u64), PlasmaError> {
        ...
        let (base_amount_deposited, quote_amount_deposited, lp_shares) = if total_shares == 0 {
            let Some(lp_shares) = initial_lp_shares.map(|s| s.upcast()) else {
                return Err(PlasmaError::MissingExpectedArgument);
            };
            ...
            if lp_shares_squared <= initial_k && lp_shares_squared + (lp_shares * 2) + 1 > initial_k
            {
                ...
                return Err(PlasmaError::UnexpectedArgument);
            }
        } else {
        ...
        if lp_shares == 0 {
            return Err(PlasmaError::BelowMinimumLpSharesRequired);
        }
    ...
    pub fn burn(&mut self, slot: SlotWindow, lp_shares: u64) -> Result<(u64, u64), PlasmaError> {
        self.maybe_update_snapshot(slot);
        ...
        if base_amount_withdrawn == 0 || quote_amount_withdrawn == 0 {
            return Err(PlasmaError::BelowMinimumWithdrawaRequired {
...
impl Amm {
    fn update_pool_reserves_after_buy(
        ...
        self.base_reserves = self
            ...
            .ok_or(PlasmaError::Underflow)?;
        self.quote_reserves = self
            .quote_reserves
            .checked_add(quote_in.downcast()?)
            .ok_or(PlasmaError::Overflow)?;
        Ok(())
    }
    ...
    fn apply_fees(&mut self, quote_fee: u128) -> Result<(), PlasmaError> {
        ...
        if lp_fees + protocol_fees != total_fees {
            return Err(PlasmaError::MismatchedFees(
                total_fees as u128,
...
impl Amm {
    pub fn buy_exact_in(
        ...
        if self.total_lp_shares == 0 {
            return Err(PlasmaError::UninitializedPool);
        }
        self.maybe_update_snapshot(slot);
    ...
    pub fn buy_exact_out(
        ...
        if self.base_reserves < base_out {
            return Err(PlasmaError::SwapExactOutTooLarge);
        }
...
impl Amm {
    pub fn sell_exact_in(
        ...
        if (base_in + base_reserves).downcast().is_err() {
            return Err(PlasmaError::SwapExactInTooLarge);
        }
